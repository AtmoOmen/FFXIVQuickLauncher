...
public static class NativeAclFix
{
    public static Process LaunchGame(string workingDir, string exePath, string arguments, IDictionary<string, string> envVars, DPIAwareness dpiAwareness, Action<Process> beforeResume)
    {
        ...
        try
        {
            if (envVars.Count > 0)
            {
                var envstr = string.Join("\0", envVars.Select(entry => entry.Key + "=" + entry.Value));
        ...
        finally
        {
            ...
            if (!Equals(lpEnvironment, IntPtr.Zero))
    ...
    private static class PInvoke
    {
        ...
        {
            private uint    grfAccessPermissions;
            private uint    grfAccessMode;
            private uint    grfInheritance;

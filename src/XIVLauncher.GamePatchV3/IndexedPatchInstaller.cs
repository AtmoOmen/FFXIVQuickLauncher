...
public class IndexedPatchInstaller : IDisposable
{
    public readonly PatchIndex              Index;
    ...
    public delegate void OnCorruptionFoundDelegate(PatchPartLocator part, PatchPartLocator.VerifyDataResult result);
    ...
    private static class PInvoke
    {
        ...
        public const uint ERROR_NOT_ALL_ASSIGNED = 0x514;
    ...
    public IndexedPatchInstaller(PatchIndex def)
    {
        Index = def;
        ...
        try
        {
            ...
            var progressMax =
                refine
                    ...
                    : Index.Targets.Select((x,                   i) => targetStreams[i] == null ? 0 : x.FileSize).Sum();
                            ...
                            () =>
                            {
                                ...
                                foreach (var partIndex in targetPartIndicesToCheck)
                                {
                                    ...
                                    lock (verifyTasks)
                                    {
                                        ...
                                        switch (verifyResult)
                                        {
                                            case PatchPartLocator.VerifyDataResult.Pass:
                                                break;

                                            case PatchPartLocator.VerifyDataResult.FailUnverifiable:
                                                throw new
                                                (
                                            ...
                                            case PatchPartLocator.VerifyDataResult.FailNotEnoughData:
                                            case PatchPartLocator.VerifyDataResult.FailBadData:
    ...
    public void QueueInstall(int sourceIndex, FileInfo file, int splitBy = 8)
    {
        ...
        for (var j = 0; j < indices.Count; j += indicesPerRequest)
        {
            ...
            (
                sourceIndex,
                file,
                new HashSet<Tuple<int, int>>(indices.Skip(j).Take(Math.Min(indicesPerRequest, indices.Count - j)))
            );
        }
    }
    ...
    public async Task Install(int concurrentCount, CancellationToken cancellationToken = default)
    {
        ...
        {
            await RepairNonPatchData(cancellationToken);
            return;
        }

        var progressMax = installTaskConfigs.Select(x => x.ProgressMax).Sum();
    ...
    {
        public readonly PatchIndex   Index;
        public readonly IndexedPatchInstaller Installer;
        ...
        protected InstallTaskConfig(IndexedPatchInstaller installer, int sourceIndex, IEnumerable<Tuple<int, int>> targetPartIndices)
        {
    ...
    public class HttpInstallTaskConfig : InstallTaskConfig
    {
        ...
        private MultipartResponseHandler? multipartResponse;

        public HttpInstallTaskConfig(IndexedPatchInstaller installer, int sourceIndex, IEnumerable<Tuple<int, int>> targetPartIndices, string sourceUrl, string? sid)
    ...
    public class StreamInstallTaskConfig : InstallTaskConfig
    {
        ...
        public StreamInstallTaskConfig(IndexedPatchInstaller installer, int sourceIndex, IEnumerable<Tuple<int, int>> targetPartIndices, Stream sourceStream)
            : base(installer, sourceIndex, targetPartIndices)
        {

...
public class DcTraveleApiException : Exception
{
    ...
    public bool RetryAfter1Min   { get; set; }

    public DcTraveleApiException(string message, int errorCode = 0)
        : base(message)
    {
...
public class DcTraveler
{
    ...
    public async Task ValidateTicket()
    {
        ...
        _ = await GetRequestData("api/gmallinter/validateTicket", ApiType.TravelWithTicket, new Dictionary<string, string> { { "ticket", ticket } }, true);
    }
    ...
    public async Task Logout()
    {
        if (isInitialized)
        {
            ...
            _ = await GetRequestData("api/gmallinter/logout?", ApiType.Order, new Dictionary<string, string>());
        }
    }
    ...
    public async Task<List<Area>> QueryGroupListTravelSource()
    {
        ...
        var areaList = JsonNode.Parse(data["groupList"].GetValue<string>()).Deserialize<List<Area>>();
    ...
    public async Task<List<Area>> QueryGroupListTravelTarget(int areaId, int groupId)
    {
        ...
        var data = await GetRequestData
                   (
                       ...
                       ApiType.Travel,
                       new Dictionary<string, string> { { "appId", "100001900" }, { "areaId", $"{areaId}" }, { "groupId", $"{groupId}" } }
                   );
        if (data["resultCode"].GetValue<int>() != 0)
    ...
    public async Task<List<Character>> QueryRoleList(int areaId, int groupId)
    {
        ...
        var characterList = JsonNode.Parse(data["roleList"].GetValue<string>()).Deserialize<List<Character>>();
        ...
        var data = await GetRequestData
                   (
                       ...
                       new Dictionary<string, string>
                       {
                           ...
                           { "areaId", $"{sourceGroup.AreaId}" }, { "areaName", $"{sourceGroup.AreaName}" }, { "groupId", $"{sourceGroup.GroupId}" }, { "groupCode", $"{sourceGroup.GroupCode}" },
                           { "groupName", $"{sourceGroup.GroupName}" },
                           { "targetArea", $"{targetGroup.AreaId}" }, { "targetAreaName", $"{targetGroup.AreaName}" }, { "targetGroupId", $"{targetGroup.GroupId}" },
                           { "targetGroupCode", $"{targetGroup.GroupCode}" }, { "targetGroupName", $"{targetGroup.GroupName}" },
    ...
    public async Task<MigrationOrders> QueryMigrationOrders(int pageIndex = 1)
    {
        ...
        var data = await GetRequestData
                       ("api/orderserivce/queryMigrationOrders", ApiType.Order, new Dictionary<string, string> { { "appId", "100001900" }, { "pageIndex", $"{pageIndex}" }, { "pageNum", "10" } });
        if (data["resultCode"].GetValue<int>() != 0)
    ...
    public async Task<string> TravelBack(string orderId, int currentGroupId, string currentGroupCode, string currentGroupName)
    {
        ...
        var data = await GetRequestData
                   (
                       ...
                       ApiType.Order,
                       new Dictionary<string, string> { { "travelOrderId", orderId }, { "groupId", $"{currentGroupId}" }, { "groupCode", currentGroupCode }, { "groupName", currentGroupName } }
                   );
        if (data["resultCode"].GetValue<int>() != 0)

...
public class SdoFileDownloadInstaller : IDisposable
{
    ...
    public void ConstructFromRemoteIntegrity(IntegrityCheck.IntegrityCheckResult remoteIntegrity)
    {
        ...
        this.downloadBaseUrl = remoteIntegrity.BaseUrl ?? throw new ArgumentException("Remote integrity must contain a download base URL.", nameof(remoteIntegrity));
        this.dataVersion = remoteIntegrity.DataVersion ?? throw new ArgumentException("Remote integrity must contain a data version.", nameof(remoteIntegrity));
        this.client.DefaultRequestHeaders.UserAgent.ParseAdd("FF14v3autopatch");
        foreach (var (relativePath, hash) in remoteIntegrity.Hashes)
        {
            if (string.IsNullOrWhiteSpace(relativePath))
                continue;

            this.targets.Add(new()
            {
    ...
    public async Task VerifyFiles(string gameRootPath, bool refine = false, int concurrentCount = 8, CancellationToken cancellationToken = default)
    {
        ...
        var brokenCandidates = this.targets
        ...
        await Parallel.ForEachAsync(brokenCandidates, parallelOptions, async (candidate, ct) =>
        {
            ...
            var reportIndex = (int)Math.Min(candidate.index, int.MaxValue);
            ...
            this.ReportVerifyProgress(reportIndex, reportCount, reportCurrent, reportMax);
        });
    }

    public void QueueInstall(int targetIndex, string filePath)
    {
        if (targetIndex < 0 || targetIndex >= this.targets.Count)
            throw new ArgumentOutOfRangeException(nameof(targetIndex));
        Log.Information($"Queueing download for {this.targets[targetIndex].RelativePath}");
        this.queuedDownloads[targetIndex] = filePath;
    }

    public async Task Install(string gameRootPath, int concurrentCount, CancellationToken cancellationToken = default)
    {
        var queue = this.queuedDownloads.ToArray();
        if (!queue.Any())
            return;
        ...
        await Parallel.ForEachAsync(queue, options, async (item, ct) =>
        {
            var target = this.targets[item.Key];
            ...
            this.ReportInstallProgress(item.Key, 0, 0, InstallTaskState.Connecting);
            var downloadUrl = GetDownloadUrl(item.Value);
            Log.Information($"Downloading {target.RelativePath} from {downloadUrl}");
            using var response = await this.client.GetAsync(downloadUrl, HttpCompletionOption.ResponseHeadersRead, ct);
            response.EnsureSuccessStatusCode();

            var totalBytes = response.Content.Headers.ContentLength ?? 0;
            var currentTotalContentBytes = Interlocked.Add(ref totalContentBytes, totalBytes);
            this.ReportInstallProgress(item.Key, Interlocked.Read(ref totalDownloadedBytes), currentTotalContentBytes, InstallTaskState.Downloading);
            ...
            try
            {
                ...
                using (var buffer = ReusableByteBufferManager.GetBuffer())
                {
                    while (true)
                    {
                        ...
                        this.ReportInstallProgress(item.Key, currentTotalDownloadedBytes, Interlocked.Read(ref totalContentBytes), InstallTaskState.Downloading);
                    }
                }
                ...
                this.ReportInstallProgress(item.Key, Interlocked.Read(ref totalDownloadedBytes), Interlocked.Read(ref totalContentBytes), InstallTaskState.Complete);
            }
            finally
            {
    ...
    private void ReportVerifyProgress(int index, int count, long progress, long max)
    {
        ...
        this.OnVerifyProgress?.Invoke(index, count, progress, max);
    }
    ...
    private void ReportInstallProgress(int index, long progress, long max, InstallTaskState state)
    {
        ...
        this.OnInstallProgress?.Invoke(index, progress, max, state);
    }
    ...
    private bool ShouldReportProgress()
    {
        ...
        {
            var minIntervalTicks = TimeSpan.FromMilliseconds(Math.Max(1, this.ProgressReportInterval)).Ticks;
            if (nowTicks - this.lastProgressTicks < minIntervalTicks)
                return false;

            this.lastProgressTicks = nowTicks;
            return true;
        }
    }
    ...
    private string GetFileKey(string filePath)
    {
        ...
        {
            var inputBytes = Encoding.Unicode.GetBytes($"{APP_ID}_{this.dataVersion}_{filePath}");
    ...
    private Uri GetDownloadUrl(string filePath)
    {
        ...
        string path = string.Join("/", pathParts.Take(pathParts.Length - 1));
        var uri = new Uri($"{this.downloadBaseUrl}/{path}/{GetFileKey(filePath)}");
    ...
    {
        return Path.Combine(gameRootPath, relativePath.TrimStart('\\'));
    }
